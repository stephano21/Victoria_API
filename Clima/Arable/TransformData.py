...
def getProduction():
    queryset = Produccion.objects.select_related(
        'Id_Proyecto__Id_Hacienda').filter(Activo=True, Id_Proyecto__Id_Hacienda_id=1)
    ...
    df = pd.DataFrame(data)
    df['date'] = pd.to_datetime(df['date'])
    df = df.groupby([df['date'].dt.to_period("M"), df['Proyecto']])[
        'qq'].sum().reset_index()
    print(df)
    return df
...
def getLotes():
    ...
    df = df.groupby([df['date'].dt.to_period("M"), df['lote'], df['densidad'], df['hectareas']])[
        ['E1', 'E2', 'E3', 'E4', 'E5']].mean().reset_index()
    dfByLote = df.copy()
   ...
   # print(dfByLote)
    ...
    df_Lote_Produccion['densidad'] = df_Lote_Produccion['densidad'].astype(
        float)
    df_Lote_Produccion['hectareas'] = df_Lote_Produccion['hectareas'].astype(
        float)
    # Aqui convierto los totales de mazorcas a quintales secos
    df_Lote_Produccion['Total_E1'] = (
        (df_Lote_Produccion['E1']*df_Lote_Produccion['densidad']*df_Lote_Produccion['hectareas'])/12)/100
    df_Lote_Produccion['Total_E2'] = (
        (df_Lote_Produccion['E2']*df_Lote_Produccion['densidad']*df_Lote_Produccion['hectareas'])/12)/100
    df_Lote_Produccion['Total_E3'] = (
        (df_Lote_Produccion['E3']*df_Lote_Produccion['densidad']*df_Lote_Produccion['hectareas'])/12)/100
    df_Lote_Produccion['Total_E4'] = (
        (df_Lote_Produccion['E4']*df_Lote_Produccion['densidad']*df_Lote_Produccion['hectareas'])/12)/100
    df_Lote_Produccion['Total_E5'] = (
        (df_Lote_Produccion['E5']*df_Lote_Produccion['densidad']*df_Lote_Produccion['hectareas'])/12)/100
    # Limpiar el dataset
    df_Lote_Produccion = df_Lote_Produccion.drop(
        ['E1', 'E2', 'E3', 'E4', 'E5', 'densidad', 'hectareas'], axis=1)
    # merge with clima
    df_weather = GetData()
    df_merged_inner = pd.merge(
        df_Lote_Produccion, df_weather, on='date', how='inner')
    df_merged_inner.to_csv('Clima.csv', index=False)

    # Agrupar por mes y sumar
    df_Total = df_Lote_Produccion.groupby(['date']).agg(
        {'Total_E1': 'sum', 'Total_E2': 'sum', 'Total_E3': 'sum', 'Total_E4': 'sum', 'Total_E5': 'sum', 'qq': 'sum'})
