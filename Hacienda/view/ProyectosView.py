...
class ProyectoAPIView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        if id_hacienda and Rol != "Researcher":
            ...
            proyectos = Proyecto.objects.select_related(
                'Id_Hacienda').filter(Activo=True)
    ...
    def post(self, request):
        ...
        if existe:
            return Response(f"El proyecto {request.data['Codigo_Proyecto']} ya existe en la hacienda", status=status.HTTP_400_BAD_REQUEST)
        if Rol != "Researcher" or Rol != "Root":
    ...
    def put(self, request, id):
        Proyecto = self.get_object(id)
        serializer = ProyectoSerializers(
            Proyecto, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            # Return the serialized lote object
            return Response(f"Proyecto {Proyecto.Codigo_Proyecto} actualizado con éxito!", status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
