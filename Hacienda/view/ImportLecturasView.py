    ...
    def post(self, request):
        ...
        if archivo_excel:
            try:
                ...
                headers = ['Planta', 'Fecha', 'E1', 'E2', 'E3', 'E4', 'E5', 'GR1',
                            'GR2', 'GR3', 'GR4', 'GR5', 'Cherelles', 'Total', 'Observacion']
                ...
                for index, row in df_copy.iterrows():
                    ...
                    lecturas_mes = Lectura.objects.filter(FechaVisita__month=fecha_visita.date(
                    ).month, FechaVisita__year=fecha_visita.date().year, Id_Planta=Id_Planta)
                    if lecturas_mes.exists():
                        print(f"{row['Planta']} ya tiene una lectura")
                        errors.append(
                            f"Error en la fila {index+1} {row['Planta']}:Ya existe una lectura de esta planta en este mes!")
                        continue
                    ...
                    serializer_data = {
                        ...
                        'Monilla': row['Monilla'],
                        # row['Usuario'] if not pd.isna(row['Usuario']) else str(username),
                    ...
                    if serializer.is_valid():
                        ...
                        errors.append(
                            f"Error en la fila {index+1} {row['Planta']}: {', '.join(list(serializer.errors.values())[0])}")
                if errors:
                    errors_str = '\n'.join(errors)
