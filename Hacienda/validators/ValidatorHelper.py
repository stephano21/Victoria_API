...
def validate_row(row, index, errors):
    ...
    if not all(row[col] and not pd.isna(row[col]) for col in headers):
        missing_data = [
            col for col in headers if not row[col] or pd.isna(row[col])]
        errors.append(
            f'Datos faltantes en la fila {index+1} : {", ".join(missing_data)}')
        has_error = True

    if len(str(row['Planta'])) <= 1 and has_error == False:
        errors.append(
            f"Error en la fila {index+1} : Código de Planta no válido!")
        has_error = True
    planta_no_existe = not Planta.objects.filter(
        Codigo_Planta=row['Planta']).exists()
    if planta_no_existe and not has_error:
        errors.append(
            f"Error en la fila {index+1} {row['Planta']}: Planta no encontrada!")
        has_error = True
    ...
    try:
        ...
        errors.append(
            f"Error en la fila {index+1} {row['Planta']}: El formato de fecha es invalido!")
        has_error = True
...
def GetIdPlanta(codigo, hacienda=None):
    try:
        codigo = codigo.strip()
        if hacienda:
